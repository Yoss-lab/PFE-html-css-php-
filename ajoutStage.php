...
//$result = $result->fetch_assoc();
  ...
  if (isset($_POST['upload'])) {
  	// Get image name
  	$image_stage = $_FILES['image_stage']['name'];
  	// Get text
	$title_stage = mysqli_real_escape_string($conn, $_POST['title_stage']);
  	$desc_stage = mysqli_real_escape_string($conn, $_POST['desc_stage']);
  	$annonce_de = mysqli_real_escape_string($conn, $_POST['annonce_de']);
  	

  	// image file directory
  	$target = "images/".basename($image_stage);

  	$sql = "INSERT INTO stage (title_stage,desc_stage,annonce_de,image_stage) VALUES ('$title_stage', '$desc_stage','$annonce_de','$image_stage')";
  	// execute query
  	mysqli_query($conn, $sql);

  	if (move_uploaded_file($_FILES['image_stage']['tmp_name'], $target)) {
...
/*
			...
			*/
			?>
			 ...
			 <form method="post" action="ajoutStage.php" enctype="multipart/form-data" class="form-horizontal row-fluid">
					
					
				   <div class="control-group">
						 <label class="control-label" for="basicinput">Titre</label>
					
						<input class="uk-input" class="span8" type="text"  name="title_stage"  id="basicinput">
					</div>
					 <div class="control-group">
					 ...
					 <input type="hidden" name="size" value="1000000" class="uk-input" class="span8">
						...
						<label class="control-label" for="basicinput">Image</label>
						  <input type="file" name="image_stage" class="uk-input" class="span8">
						</div>
